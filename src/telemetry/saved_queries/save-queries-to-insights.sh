#!/bin/bash
# Bash script to save all KQL queries to Application Insights
# Usage: ./save-queries-to-insights.sh
# This script uses the dev environment configuration from env/.env.dev

ENV_FILE="../../env/.env.dev"

echo "🚀 Saving KQL queries to Application Insights..."
    ...
    if [ -d "$category" ]; then
        ...
        for query_file in "$category"/*.kql; do
            if [ -f "$query_file" ]; then
                ...
                echo -n "  💾 Saving: $full_query_name"
                
                # Save query to Application Insights
...
echo ""
echo "🎉 Completed! Saved $successful_queries out of $total_queries queries to Application Insights."
...
if [ $successful_queries -lt $total_queries ]; then
    echo ""
    echo "⚠️  Some queries failed to save. This might be due to:"
