# PowerShell script to save all KQL queries to Application Insights
# Usage: .\save-queries-to-insights.ps1
...
Write-Host "🚀 Saving KQL queries to Application Insights..." -ForegroundColor Green
...
foreach ($category in $categories) {
    if (Test-Path $category) {
        ...
        foreach ($query in $queries) {
            ...
            Write-Host "  💾 Saving: $fullQueryName" -NoNewline
            
            try {
                # Save query to Application Insights using Log Analytics workspace
...
Write-Host "`n🎉 Completed! Saved $successfulQueries out of $totalQueries queries to Application Insights." -ForegroundColor Green
...
if ($successfulQueries -lt $totalQueries) {
    Write-Host "`n⚠️  Some queries failed to save. This might be due to:" -ForegroundColor Yellow
